...
router.post('/generate-fb-post', async (req, res) => {
    ...
    try {
        ...
        const lengthPresets = {
            short:  { rule: '1–2 věty, max ~200 znaků — jeden hutný odstavec (hook a pointa)', structure: '1 odstavec', emoji: '1',   maxTokens: 200 },
            medium: { rule: '2–4 věty rozdělené do 2 krátkých odstavců',                        structure: '2 odstavce',  emoji: '1–2', maxTokens: 450 },
            long:   { rule: '5–8 vět rozdělených do 3–4 kratších odstavců',                     structure: '3–4 odstavce', emoji: '2–3', maxTokens: 800 }
        };
...
FORMÁT:
- ${preset.rule}
- Struktura: ${preset.structure}, oddělené prázdným řádkem (dvojitým \\n)
- Každý odstavec má vlastní myšlenku (např.: 1) hook a výsledek, 2) klíčové detaily/jména, 3) poděkování/pozvánka)
- Krátké věty, snadno skenovatelné — FB čtenář scrolluje rychle

STYL:
- Český přirozený mluvený tón, ne formální zpravodajství
- První věta = hook: konkrétní a lákavé (skóre, dramatický moment, výjimečný výkon)
...
- Zakonči jemnou pozvánkou k přečtení (např. „Celý zápis a partie na webu ⬇")

VÝSTUP: POUZE text postu, bez uvozovek, bez komentáře, bez markdownu.`;
