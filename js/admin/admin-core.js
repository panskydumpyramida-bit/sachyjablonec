...
function switchTab(tab) {
    ...
    // Hide all views
    ['dashboard', 'editor', 'members', 'users', 'messages', 'blicak', 'competitions', 'gallery', 'games', 'events', 'timeline', 'settings', 'puzzleRacer', 'chessdb', 'changelog', 'weeklyPuzzle'].forEach(v => {
    ...
    else if (tab === 'games') { loadRecordedGames(); if (window.loadDiagrams) loadDiagrams(); if (window.loadFragments) loadFragments(); }
    else if (tab === 'events' && window.AdminEvents) AdminEvents.init();
    else if (tab === 'timeline' && window.AdminTimeline) AdminTimeline.load();
